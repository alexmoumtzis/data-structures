...
public class BTree implements SearchInsert{
	...
	private int T;   //minimum degree of the tree(minimum number of children)

	public BTree(int t) {
		T = t;                 
		root = new Node(T);       //create root node
		root.n = 0;           //number of keys in the root node
		root.leaf = true;    //mark root node as a leaf
	} 
	...
	private Node Search(Node x, int key) {
		MultiCounter.increaseCounter(1); // Increment the counter each time a node is penetrated
		int i = 0;
		if (x == null)
			return x;
		for (i = 0; i < x.n; i++) {
